...
private:
	...
	static constexpr unsigned RX_SLOTS = 32;
	static constexpr unsigned RX_SLOT_BYTES = 1536;
	static constexpr unsigned TX_STATUS_DEPTH = 31;

	// 32 KiB packet SRAM, split 1:1 between the RX and TX partitions (the
	// "practical default" noted in MAME_3C509B_ISA8.md 6.2); used only to
	// compute the Free-byte registers, since RX storage itself is capped
	// by the fixed RX_SLOTS ring rather than modeled as a byte-addressed
	// SRAM window.
	...
	// RX: fixed-capacity ring of whole frames (save-state friendly). A
	// frame is only removed from the head by an explicit RX_DISCARD
	// command; reading its data via the FIFO never advances the ring.
	uint8_t m_rx_data[RX_SLOTS][RX_SLOT_BYTES];
	...
	uint8_t m_rx_stage_data[RX_SLOT_BYTES];
